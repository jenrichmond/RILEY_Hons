...
```{r}
confint.merMod(child_brow_emo_bin_interaction, level = 0.95)
```

```{r}

really_nice_table <- dust(child_brow_emo_bin_interaction) %>%
...
```{r}
tidy_cheek_adult <- tidy(adult_brow_emo)
```
...
tidier_cheek_adult <- tidy_cheek_adult %>%
  filter(effect == "fixed") %>%
  select(-group) 


tidier_cheek_adult <-  tidier_cheek_adult %>%
  mutate(niceterms = c("intercept", "happy vs. angry"))
...
#get confidence intervals 
```{r}
confint.merMod(adult_brow_emo_bin_interaction, level = 0.95)
```

```{r}

really_nice_table <- dust(adult_brow_emo_bin_interaction) %>%
  sprinkle(col = 4:7, round = 3, pad = 15, halign = "center", valign = "middle") %>%
  sprinkle(col = 8, fn = quote(pvalString(value)), halign = "center", valign = "middle") %>%
  sprinkle_colnames(term = "Term", 
                    estimate = "Estimate", 
